...
set dirPath=".chaozhuo.gameassistant2"
...
:step2
set "ret=0"
for /f %%i in ('adb shell ls /sdcard/%dirPath%') do (
echo %%i | findstr "inject.sh"
if %ERRORLEVEL% == 0 set "ret=1"
)
if %ret%==0 goto step2_1
if %ret%==1 goto step3

:step2_1
echo Wsh.Echo MsgBox("Please launch APP 'Panda Gamepad Pro' or 'Panda Mouse Pro' first on your phone and click Retry.",vbRetryCancel,%msgTitle%) >tmp.vbs
for /f %%i in ('cscript tmp.vbs //nologo //e:vbscript') do (set "ret=%%i")
del /q tmp.vbs
...
:step3
set "ret=0"
adb shell sh /sdcard/%dirPath%/inject.sh
